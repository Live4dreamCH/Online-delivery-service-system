...
int main()
{
	...
	do	//不断循环，从主界面开始
	{
		...
		else if (n == 3)		//v3.0
		{
			...
			{
				system("cls");	//清空窗口用
				bool all = true;	//v4.0，用于判断是否所有用户都没有包裹可以展示，如果全都没有，才输出"无过往订单！"
				for (int i = 0; i < c.size(); i++)	//v3.0这里，如果有任何一个顾客无过往订单，就会输出”无过往订单“，然而事实上需要全部顾客都没有订单才需要输出，v4.0有修改
				{
					if (c[i].shownow())	//只要有人有包裹，就不输出"无过往订单！"
						all = false;
						//cout << "无过往订单！\n";
				}
				if(all)
